...
resource "null_resource" "docker_build" {
    ...
    #도커 태그 사용하려면,
    #provisioner "local-exec" {
    #    command = "docker build -t ${local.service_name}:${var.app_version} --file ../${local.service_name}/Dockerfile-prod ../${local.service_name}"
    #}
}
...
#
#    provisioner "local-exec" {
#        command = "docker tag ${local.service_name}:${var.app_version} ${local.image_tag}"
#    }
#}
